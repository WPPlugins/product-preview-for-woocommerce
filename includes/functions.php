...
if( ! function_exists( 'br_build_preview' ) ) {
    function br_build_preview($name) {
        if ( $name == 'image' ) {
            do_action( 'berocket_pp_popup_before_image' ); ?>
            <a class="berocket_preview_image" href="<?php the_permalink(); ?>">
                <?php woocommerce_template_loop_product_thumbnail(); ?>
            </a>
            <?php do_action( 'berocket_pp_popup_after_image' );
        } elseif ( $name == 'title' ) {
            do_action( 'berocket_pp_popup_before_title' ); ?>
            <a class="berocket_preview_title" href="<?php the_permalink(); ?>">
                <h3><?php the_title(); ?></h3>
            </a>
            <?php do_action( 'berocket_pp_popup_after_title' );
        } elseif ( $name == 'buttons' ) {
            do_action( 'berocket_pp_popup_before_buttons' );
            woocommerce_template_loop_add_to_cart();
            do_action( 'berocket_pp_popup_after_buttons' );
        } elseif ( $name == 'description' ) {
            do_action( 'berocket_pp_popup_before_description' );
            woocommerce_template_single_excerpt();
            do_action( 'berocket_pp_popup_after_description' );
        } elseif ( $name == 'meta' ) {
            do_action( 'berocket_pp_popup_before_meta' ); ?>
            <div class="berocket_preview_meta">
                <?php woocommerce_template_single_meta(); ?>
            </div>
            <?php do_action( 'berocket_pp_popup_after_meta' );
        } elseif ( $name == 'price' ) {
            do_action( 'berocket_pp_popup_before_price' ); ?>
            <div class="berocket_preview_price">
                <?php woocommerce_template_loop_price(); ?>
            </div>
            <?php do_action( 'berocket_pp_popup_after_price' );
        }
    }
}
if( ! function_exists( 'br_product_preview_general_callback' ) ) {
    /**
     * Function for general settings callback
     *
     * @return void
     */
    function br_product_preview_general_callback() { 
        include Product_Preview_TEMPLATE_PATH . "general_settings.php";
    }
}

if( ! function_exists( 'br_product_preview_style_callback' ) ) {
    /**
     * Function for style settings callback
     *
     * @return void
     */
    function br_product_preview_style_callback() { 
        include Product_Preview_TEMPLATE_PATH . "style_settings.php";
    }
}

if( ! function_exists( 'br_product_preview_text_callback' ) ) {
    /**
     * Function for text settings callback
     *
     * @return void
     */
    function br_product_preview_text_callback() { 
        include Product_Preview_TEMPLATE_PATH . "text_settings.php";
    }
}

if( ! function_exists( 'br_product_preview_javascript_callback' ) ) {
    /**
     * Function for javascript settings callback
     *
     * @return void
     */
    function br_product_preview_javascript_callback() { 
        include Product_Preview_TEMPLATE_PATH . "javascript_settings.php";
    }
}

if( ! function_exists( 'br_product_preview_license_callback' ) ) {
    /**
     * Function for license settings callback
     *
     * @return void
     */
    function br_product_preview_license_callback() { 
        include Product_Preview_TEMPLATE_PATH . "license_settings.php";
    }
}
...
if ( ! function_exists( 'berocket_font_select_upload' ) ) {
    ...
    function berocket_font_select_upload( $text, $id, $name, $value, $show_fa = true, $show_upload = true, $show_remove = true, $data_sc = '' ) {
        ...
        if ( $show_fa ) {
            $result .= '<input type="button" class="berocket_aapf_font_awesome_icon_select button" value="'.__('Font awesome', 'BeRocket_Product_Preview_domain').'"/>
        ...
        if ( $show_upload ) {
            $result .= '<input type="button" class="berocket_aapf_upload_icon button" value="'.__('Upload', 'BeRocket_Product_Preview_domain').'"/> ';
        }
        if ( $show_remove ) {
            $result .= '<input type="button" class="berocket_aapf_remove_icon button" value="'.__('Remove', 'BeRocket_Product_Preview_domain').'"/>';
        }
        $result .= '</div>';
